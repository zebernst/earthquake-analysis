---
title: "Earthquakes"
author: "Zach Bernstein, Zack Vanden Bergh, and Nick Hanoian"
date: "11/30/2017"
output:
...
```{r map_allquakes, echo=FALSE, fig.width=9, fig.align='center'}
...
```{r map_highmag, echo=FALSE, warning=FALSE, fig.width=9, fig.align='center'}
quakesPower <- quakes %>%
  filter(mag > 5.5) %>% 
  mutate(power = factor(ifelse(mag < 6, 1, 
                          ifelse(mag < 6.5, 2, 3))))
...
The trends of high activity areas from the first location-based graph are seen again. The Ring of Fire area, especially around Indonesia and  Papa New Guinea, has a large number of high-intensity earthquakes, as does the western coast of the Americas. The large number of earthquakes that were previously seen across the United States are no longer seen, because they were all low magnitude earthquakes.

### Distribution of Quake Magnitude by Significant Data Contributor

```{r network, echo=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.width=8, fig.align='center'}
quakes_net_filtered <- quakes %>% 
  group_by(net) %>% 
  filter(n() > 250) %>% 
  mutate(netNamed = ifelse(net=='ak', 'AK',
                           ifelse(net=='ci', 'CI',
                                  ifelse(net=='hv', 'HV',
                                         ifelse(net=='mb', 'MB',
                                                ifelse(net=='nc', 'NC',
                                                       ifelse(net=='nn', 'NN',
                                                              ifelse(net=='pr', 'PR',
                                                                     ifelse(net=='us', 'INT',
                                                                            ifelse(net=='uu', 'UU',
                                                                                   ifelse(net=='uw', 'UW', 'na')))))))))))  # capitalizes names of contributors for facet titles

contrib_names <- c("AK [Alaska Earthquake Center]", "CI [California Integrated Seismic Network]", "HV [Hawaii Volcano Observatory]", "MB [Montana Bureau of Mines and Geology]", "NC [Northern California Seismic System]", "NN [Nevada Seismological Laboratory]", "PR [Puerto Rico Seismic Network]", "INT [Gathered from international sources]", "UU [University of Utah Seismograph Stations]", "UW [Pacific Northwest Seismic Network]")
...
```{r bimodal, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
...
```{r us_quakes, message=FALSE, warning=FALSE, echo=FALSE, fig.width=9, fig.align='center'}
quakes_us <- quakes %>% 
...
```{r us_density, warning=FALSE, message=FALSE, echo=FALSE, fig.align='center'}
...
```{r water-based, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
seabased <- quakes %>% 
  filter(tsunami==1)
